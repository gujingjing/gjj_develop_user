...
//        void handle(String time);
    ...
    public enum Type {
        ...
        YEAR_MONTH_DAY, //年月日
        YEAR_MONTH,//年月
        YEAR,//年  addBy Gjj 2015-12-03
        HOURS_MINS, //时分
        MONTH_DAY_HOUR_MIN ,//月日时分
    }// 四种选择模式，年月日时分，年月日，年月,年，时分，月日时分
    ...
    public TimeSelector(Context context, ResultHandler resultHandler, long startDate, long endDate,Type type) {
        this.context = context;
        this.handler = resultHandler;
        this.type=type;
        startCalendar = Calendar.getInstance();
    ...
    public TimeSelector(Context context, ResultHandler resultHandler, String startDate, String endDate,Type type) {
        this.context = context;
        this.handler = resultHandler;
        this.type=type;
        startCalendar = Calendar.getInstance();
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            ...
            //addBy gjj 2015-12-08
            seletorDialog.setCanceledOnTouchOutside(true);//点击其他部位可以取消
...
//                handler.handle(DateUtil.format(selectedCalender.getTime(), FORMAT_STR));
    ...
    private void initView(Type type) {
        year_pv = (PickerView) seletorDialog.findViewById(R.id.year_pv);
        tvyear= (TextView) seletorDialog.findViewById(R.id.year);

        month_pv = (PickerView) seletorDialog.findViewById(R.id.month_pv);
        tvmonth= (TextView) seletorDialog.findViewById(R.id.month);

        day_pv = (PickerView) seletorDialog.findViewById(R.id.day_pv);
        tvday= (TextView) seletorDialog.findViewById(R.id.day);

        hour_pv = (PickerView) seletorDialog.findViewById(R.id.hour_pv);
        tvhour= (TextView) seletorDialog.findViewById(R.id.hour);

        minute_pv = (PickerView) seletorDialog.findViewById(R.id.minute_pv);
        tvminute= (TextView) seletorDialog.findViewById(R.id.minute);
